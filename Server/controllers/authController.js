...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const url = `${req.protocol}://${req.get("host")}/mon-compte`;
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(
      new AppError("Merci de fournir un email et un mot de passe", 400)
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError("Il n'y a pas d'utilisateur avec cette adresse email.", 404)
    );
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  // User.findByIdAndUpdate ne fonctionnerai pas car pas de vérif si pas "save"
